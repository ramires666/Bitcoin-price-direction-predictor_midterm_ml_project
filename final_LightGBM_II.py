...
def add_essential_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Добавляет микроструктурные и волатильностные фичи:
    - bid/ask-цены и спред (bid_ask_spread, relative_spread),
    - OFI (order flow imbalance) и его кумулятивную сумму (cumulative_ofi),
    - лог-доходность (log_return),
    - реализованную волатильность на окнах 8 и 16 (realized_volatility_8/16).
    """
    # Спред
    df["bid_price"] = df["low"] + (df["close"] - df["low"]) * 0.3
    df["ask_price"] = df["high"] - (df["high"] - df["close"]) * 0.3
    df["bid_ask_spread"] = df["ask_price"] - df["bid_price"]
    df["relative_spread"] = (df["bid_ask_spread"] / (df["bid_price"] + df["ask_price"]) / 2) * 100

    # OFI - главный микроструктурный сигнал
    total_vol = df["ask_vol"] + df["bid_vol"]
    df["ofi"] = (df["ask_vol"] - df["bid_vol"]) / (total_vol + 1e-8)
    df["cumulative_ofi"] = df["ofi"].rolling(8).sum().shift(1)

    # Возвраты
    df["log_return"] = np.log(df["close"] / df["close"].shift(1)).fillna(0)

    # Волатильность
    df["realized_volatility_8"] = df["log_return"].rolling(8).std()
    df["realized_volatility_16"] = df["log_return"].rolling(16).std()
...
def add_lag_features(df: pd.DataFrame, n_lags: int = 12) -> pd.DataFrame:
    """
...
# ==============================
# 7. ПОДГОТОВКА ПРИЗНАКОВ + ROBUSTSCALER + TRAIN/VALID/TEST SPLIT
# ==============================
...
def prepare_features_and_split(
    ...
    """
    Шаг 5. Подготовка данных, временной train/valid/test split и нормализация.

    КЛЮЧЕВОЕ ИЗМЕНЕНИЕ:
    - Данные делятся на 3 части: train (обучение), validation (подбор гиперпараметров),
      и test (финальная оценка). Это предотвращает утечку информации с тестовой
      выборки в процесс подбора модели.

    Подшаги:
    1) Сдвинуть все фичи, которые завязаны на текущий бар, на 1 назад.
    2) Удалить строки с NaN.
    3) Установить 'time' как индекс.
    4) Сформировать список признаков.
    5) Сделать временной split на train/validation/test (например, 70%/15%/15%).
    6) Обучить RobustScaler ТОЛЬКО на train и применить его к validation и test.
    """

    # 7.1. Копируем датафрейм
    df = df.copy()

    # 7.2. Сдвигаем фичи на 1 бар назад для предотвращения утечки
    ...
    # 7.5. Список признаков = все колонки, кроме целевой
    ...
    # 7.7. Временной split на train, validation, test
    ...
    X_valid = X.iloc[train_split_index:valid_split_index]
    y_valid = y.iloc[train_split_index:valid_split_index]

    X_test = X.iloc[valid_split_index:]
    y_test = y.iloc[valid_split_index:]

    # 7.8. Обучаем RobustScaler только на обучающей части и трансформируем все сеты
    scaler = RobustScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_valid_scaled = scaler.transform(X_valid)
    X_test_scaled = scaler.transform(X_test)

    # 7.9. Сохраняем индексы тестовой части для визуализации
    ...
    return (
        ...
        feature_columns,
        test_indices,
        df  # df уже после всех преобразований
    )
...
def run_training_pipeline():
    ...
    2) Добавляет объёмные, технические, микроструктурные и лаговые фичи.
    3) Формирует целевую переменную y.
    4) Делает временной split на train/validation/test (70%/15%/15%) и нормализует признаки.
    5) Выполняет ручной грид-сёрч гиперпараметров LightGBM на валидационной выборке.
    6) Оценивает лучшую модель на тестовой выборке.
    7) Сравнивает с random walk.
    8) Визуализирует результаты.
    """
    ...
    # --- 3.2. Микроструктурные и волатильностные фичи ---
    print("Шаг 3.2: Добавление микроструктурных и волатильностных фич...")
    df = add_essential_features(df)

    # --- 3.1. Лаги цены и объёмов ---
    print("Шаг 3.1: Добавление лаговых фич...")
    df = add_lag_features(df, n_lags=12)

    # --- 4. Целевая переменная y ---
    horizon = 4
    print(f"Шаг 4: Формирование целевой переменной y (горизонт t+{horizon})...")
    df = add_target(df, horizon=horizon)

    # --- 5. Подготовка признаков + RobustScaler + train/valid/test split ---
    print("Шаг 5: Подготовка признаков, RobustScaler, train/valid/test split...")
    (
        X_train_scaled,
        y_train,
        X_valid_scaled,
    ...
    # --- 6. Обучение LightGBM + ручной грид-сёрч ---
    print("Шаг 6: Обучение LightGBM-модели и грид-сёрч...")
    ...
    grid_result = grid_search_xgboost_gpu(
        ...
        y_train=y_train,
        X_valid=X_valid_scaled,  # Используем валидационный сет для грид-сёрча
        y_valid=y_valid,
    ...
    model_eval = evaluate_model(
        model=bst,
        X_test=X_test_scaled, # Используем тестовый сет для финальной оценки
        y_test=y_test
    )
    y_pred = model_eval["y_pred"]

    # --- 8. Оценка random walk ---
    print("Шаг 8: Оценка random walk baseline...")
    ...
    # --- 9. Восстановление цен ---
    print("Шаг 9: Восстановление цен из лог-доходности...")
    ...
    # --- 10. Визуализация ---
    print("Шаг 10: Визуализация результатов...")
...
if __name__ == "__main__":
    ...
    # Для запуска второй версии LightGBM-скрипта:
    #   python final_LightGBM_II.py
    #
    # или:
    #   from final_LightGBM_II import run_training_pipeline
    #   run_training_pipeline()
    run_training_pipeline()